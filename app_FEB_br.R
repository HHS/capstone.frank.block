                        ...
                        navbarPage("Menu",
                                   ...
                                   navbarMenu("Exploratory Analysis",
                                              ...
                                              tabPanel("Frequent Terms", 
                                                       ...
                                                       # Frequent Terms ####
                                                       box("Frequent Terms", #this is where the slider for frequent terms is adjusted
                                                           width = 16,
...
server = function(input, output, session) {
    ...
    # If unset, the maximum request size defaults to 5MB, hence the adjustement to 800MB
    ...
    string_stopwords_filter05 <- reactive({
        ...
        sparse_string <- dfm(string3, #this is where stopwords are filtered and words are stemmed, if desired
    ...
    output$keyword_choice <- renderUI({
        if (is.null(string1())) {
            ...
            selectInput(inputId = "s?sorelected_keyword",
    ...
    add_device_category <- function(dataframe){
        ...
        dataframe$device_category[grepl("esthe", dataframe$device_type)] = 2 #anes system
    ...
    data_proc_merge <- function(dataframe=pump, to_lower=T, lemmatize=T, remove_punct=T, remove_digit=T){
        ...
        # Replace synonyms
        # function may not be needed???? - FEB
...
shinyApp(ui, server) #### Run the program